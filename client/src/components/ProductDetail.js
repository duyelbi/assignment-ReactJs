...
function ProductDetail() {
    ...
    const api = `http://localhost:9000/products/${id}`;
    ...
    useEffect(() => {
        axios.get(api)
            .then(response => setState({ product: response.data }));
    }, [])

    const src = `${state.product.image}`;
    ...
    function imgProduct() {
        ...
        // const srcImage = srcImg.substring(0, srcImg.length).split(",");
        // console.log(srcImage);
        console.log(state.srcImage)
        return (
            <div className="item-slick3" data-thumb={"upload/" + { srcImage }}>
                <div className="wrap-pic-w pos-relative">
                    <img src={"upload/" + { srcImage }} alt="IMG-PRODUCT" />
                    <a className="flex-c-m size-108 how-pos1 bor0 fs-16 cl10 bg0 hov-btn3 trans-04" href={"upload/" + { srcImage }}>
            ...
            <section className="sec-product-detail bg0 p-t-65 p-b-60">
                ...
                <div className="bg6 flex-c-m flex-w size-302 m-t-73 p-tb-15">
                    <span className="stext-107 cl6 p-lr-25">
                        Tag: {state.product.tag.map((tag => <p>{tag}</p>))}
                    </span>
                    <span className="stext-107 cl6 p-lr-25">
                        Categories: {state.product.cate.map((cate => <p>{cate}</p>))}
                    </span>
                </div>
            </section>
            {/* Related Products */}
